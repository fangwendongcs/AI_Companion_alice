...
后续工作不再继续无限扩展 Phase 4。短期路线切回 Phase 5 的记忆、人格和陪伴连续性主线；SQLite、长期 Memory、persona 和中文陪伴体验才是近期重点。RAG / Qdrant / embedding 保留为可选增强，不作为 Phase 5 近期主线。安全相关增强按需要分层推进：
...
- TTS provider key：`QWEN3_TTS_API_KEY` / `DASHSCOPE_API_KEY` / 同 region `QWEN_API_KEY`、`FISH_AUDIO_API_KEY`、`MINIMAX_API_KEY`、`COSYVOICE_API_KEY`、`HIGGS_API_KEY`
...
- provider API Key
- TTS API Key，包括 `QWEN3_TTS_API_KEY` / `DASHSCOPE_API_KEY` / 同 region `QWEN_API_KEY`、`FISH_AUDIO_API_KEY`、`MINIMAX_API_KEY`、`COSYVOICE_API_KEY`、`HIGGS_API_KEY`
- `N8N_WEBHOOK_URL`
- `N8N_WEBHOOK_SECRET`
