...
Phase 3 已完成智能能力基线，但这仍然不是生产部署安全基线。当前 `stub`、SQLite-backed 短期 Memory、保守长期 `memory_items`、本地 RAG、n8n 工具边界和 Agent pipeline 都默认服务于本地演示与开发验收。
...
- 当前 Memory 已使用 SQLite 保存最近会话，并支持保守长期 `memory_items`。长期记忆只能保存用户明确要求记住的稳定信息，不能保存 API Key、密码、身份证、金融信息、token、secret 或高敏隐私；后续记忆管理 UI 前仍需继续补清除、导出、保留期限和隐私提示。
