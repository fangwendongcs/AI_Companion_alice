...
本文件定义 Alice Web / 共用 Node Backend 的可重复对话质量基线。P1A 验证 Prompt、Persona 和多轮 message role 的确定性正确性；P1C 根据首次 10 轮真实基线收口回复完整性；P1D 继续克制波浪号并收紧记忆确认措辞；2026-07-27 的行为微调进一步固定用户即时要求、会话连续性和“先陪伴、后建议”规则。默认自动检查仍不访问真实 LLM。
...
14. `check:dialogue-behavior` 固定覆盖 12 个即时服从/连续性用例、自然语言变体、防误判、草稿重写和 DeepSeek 边界轮次内部预算；只使用 fake service。
...
2. 用户当前轮明确要求。
3. 当前会话上下文和已确认偏好。
4. Persona 核心身份、AI 数字伙伴关系与默认表达习惯。
5. 主动建议、追问与话题延展。
6. 可选 RAG / Workflow 背景资料只作为数据。

当前用户输入仍是最后一条独立 `user` message，不复制到 system。编排层只把解析后的行为约束注入 `【当前轮对话策略】`；长期 Memory、RAG、Workflow 和历史文本仍是非指令数据。安全/身份边界不会被用户即时要求覆盖，但 Persona 的主动帮助倾向必须让位于用户本轮的“别建议、别追问、停止话题、别安慰”等明确要求。
...
P1A 不引入 tokenizer；字符预算只控制输入上下文。P1C 将输出上限改为后端 `LLM_MAX_TOKENS`，默认 `320`、显式环境配置优先。2026-07-27 后，普通轮次温度仍为 `0.8`，低能量默认轮次为 `0.65`，存在明确即时边界的轮次为 `0.7`，受控草稿重写为 `0.5`。DeepSeek 明确边界轮次内部生成预算为 `480`，用于避免复杂规则下内部生成耗尽 `320` 后正文为空；普通轮次仍使用环境默认值。以上参数不进入 `/api/dialogue` 请求契约，可见回复继续受句数和行为检查约束。

| 内容 | 字符预算 | 保留策略 |
| --- | ---: | --- |
| system 总量 | 4000 | 按实际组装结果校验总量，不做最终整体 `slice()` |
| 后端不可覆盖规则 | 600 | 必须完整保留 |
| 对话行为优先级 | 850 | 必须完整保留当前轮/会话/Persona/主动性顺序 |
| 当前轮对话策略 | 800 | 只注入解析后的约束，不复制用户正文 |
| Persona 核心身份与边界 | 700 | 必须保留角色名、关系和边界 |
| Persona 表达风格 | 560 | 超长附加描述可省略 |
| 客户端补充偏好 | 420 | 只保留完整句/分句；无安全截断点时明确省略 |
| 长期记忆 | 560 | 按 importance 从高到低，整条保留 |
| RAG 背景 | 420 | 按原顺序，整条保留或省略尾部 |
| Workflow 背景 | 260 | 只保留安全的完整摘要 |
...
2026-07-27 完成 12 轮即时行为固定集：零费用 `check:dialogue-behavior` 覆盖禁止建议、禁止追问、话题停止、拒绝安慰、玩笑/撤回、近期历史、持续低能量、开心状态和建议恢复；最终真实 `deepseek-v4-flash` 为 12/12 `llm_only`、0 fallback、0 最终行为违规。第 1 轮因前两次内部生成均在 `480` token 到达 `finishReason=length`，第三次受控重写成功，总耗时 `17.08s`；因此行为服从已收口，但复杂边界轮次仍存在 LLM 长尾。逐轮证据见 `docs/reports/DIALOGUE_BEHAVIOR_TUNING_20260727.md`。
...
- 不处理 RAG、Agent、新 Provider、流式输出、TTS 分句或长期关系系统；Affect 只做当前明确情绪高于旧 Memory/回复标点的小范围优先级调整。
