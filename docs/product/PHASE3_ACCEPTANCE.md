...
- `options.useMemory=true` 时返回 `memory.used=true` 或明确 `not_configured`。
- Memory 不写入前端 localStorage。
- Memory 不落入 `public/`。
- 重启服务后临时 memory 可清空。

## RAG 验收

- Phase 3.5 先验收本地 markdown / JSON 小型检索。
- 知识库文件不放入 `public/`。
- `options.useRag=false` 时行为与阶段 2 一致。
- `options.useRag=true` 时可返回 `sources`。
- 没有命中时不报错、不阻塞回复。
- RAG 片段有长度上限。
