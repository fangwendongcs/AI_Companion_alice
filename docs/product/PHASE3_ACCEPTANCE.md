...
- Phase 3.5 先验收本地 markdown / JSON 小型检索边界。
...
- Phase 3.5 默认不把本地检索接入 `/api/dialogue` 的真实回复链路；`options.useRag=true` 在主接口中仍保持 `not_configured`。
- Phase 3.6 才要求 `options.useRag=true` 返回 `sources`。
- 没有命中时不报错、不阻塞回复。
- RAG 片段有长度上限。
...
- 编排顺序清晰：Memory -> RAG -> optional workflow -> LLM。
- 每个子能力可独立关闭。
- 子能力失败时有降级策略。
