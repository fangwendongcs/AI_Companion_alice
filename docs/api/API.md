...
| `/api/tts/providers/:id/config` | `GET` / `PUT` | Protected safe-read / encrypted-save boundary for remote and self-hosted TTS configuration. Secret values are never returned. |
| `/api/tts/providers/:id/test` | `POST` | Protected unsaved configuration test. It calls only the target adapter and disables local fallback. |
...
Current Web Settings select `cosyvoice`, `qwen3_tts`, `fish_audio`, or `self_hosted`; `mock` remains a public diagnostic/test descriptor with `selectable=false`. Ordinary synthesis clients send only a public provider id plus `text`, `locale`, `emotion`, `tone`, `prosody`, and `stream`. The protected Debug Settings flow is the only client configuration exception: it sends temporary provider fields to Test/Save endpoints, never persists a Key in localStorage, and never receives saved secret plaintext. CosyVoice2 defaults to the official FastAPI runtime contract (`/inference_sft` by default). Qwen3-TTS uses Alibaba Cloud Model Studio / DashScope's native multimodal-generation API, Fish Audio uses native `/v1/tts`, and self-hosted uses an OpenAI-compatible speech adapter. Remote/self-hosted failure first falls back to the default local CosyVoice2. Qwen3/Fish still require live acceptance.
...
- Web and iOS do not store provider keys in ordinary frontend persistence. Trusted Debug Settings can submit a temporary password field to the protected backend, which encrypts saved configuration and returns only secret configured state.
