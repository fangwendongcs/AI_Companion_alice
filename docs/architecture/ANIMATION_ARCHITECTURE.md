...
```json
{
  "slots": {
    "headTap": {
      "file": "public/models/animations/head.fbx",
      "loop": "once",
      "priority": 10,
      "layer": "gesture",
      "interrupt": true,
...
旧字段 `motionManifest` 和 `skeletonMap` 仍然保留，保证当前代码和旧角色配置兼容；旧 `meta.json` 只作为 legacy fallback。
...
1. 确认角色是否加载成功：`public/avatars/registry.json` 和对应 `manifest.json`。
2. 确认 `manifest.json` 的 `motionManifest` 或 `animations.manifest` 路径正确。
